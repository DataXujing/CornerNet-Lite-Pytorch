...
def train(training_dbs, validation_db, system_config, model, args):
    ...
    nnet = NetworkFactory(system_config, model, distributed=distributed, gpu=gpu)  # 构建模型
    if initialize:
        nnet.save_params(0)
        exit(0)
...
def main(gpu, ngpus_per_node, args):
    ...
    cfg_file = os.path.join("./configs", args.cfg_file + ".json") #配置文件的路径！
    ...
    # 模型的名字作为模型导入
    model_file = "core.models.{}".format(args.cfg_file)  # 模型路径
    model_file = importlib.import_module(model_file)     # importlib.import_module，导入模型。
    ...
    training_dbs = [datasets[dataset](config["db"], split=train_split, sys_config=system_config) for _ in
                    range(workers)]   ## 涉及图片读取，标注文件读取
    validation_db = datasets[dataset](config["db"], split=val_split, sys_config=system_config) ## 打印1次“start prefetching data...”

    if rank == 0:
        print("system config...")
        pprint.pprint(system_config.full)  # 打印system config

        print("db config...")
        pprint.pprint(training_dbs[0].configs)  # 打印db config

        print("len of db: {}".format(len(training_dbs[0].db_inds)))  #训练集的图片数
        print("distributed: {}".format(args.distributed))   #是否分布式训练
